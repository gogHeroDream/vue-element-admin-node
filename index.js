...
const JwtUtil = require('./models/jwt');
        ...
        req.addListener("end", function () {
            ...
            if (pathName == '/user/login') {
                ...
                if (!username || !password) {
                    ...
                    endData.message = username.length > 10 ? '用户名不能大于10位' : '密码不能大于20位';
                    res.write(ToJsonStr(endData))
                    res.end()
                } else {
                    new Promise((resolve, reject) => {
                        ...
                        connection.query(readSql, (err1, res1) => {
                            if (err1) {
                                ...
                                let newRes = JSON.parse(ToJsonStr(res1));
                                // console.log(newRes, res1, username)
                                if (!newRes || newRes.length === 0) {
                                    endData.message = '不存在该用户！'
                                    res.write(ToJsonStr(endData))
                                    res.end()
                                } else if (newRes[0].pwd !== password) {
